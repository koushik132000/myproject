...
//Modified Date:21Feb2025
	...
	//10-07-2025: Added MPI parallelization
	//26-07-2025: Corrected output file names to include rank
	//01-08-2025: Corrected to run for multiple seed values
...
int main(int argc, char **argv)
{
	...
	char chain_file[30];
	//char file_number[3]; no need for this
	...
	long seed = (-12345)*rank; // Example fixed seed, can be modified as needed

	if (seed>=0)
		seed=-1-seed;

	printf("Rank %d using seed %ld\n", rank, seed);
	...
	sprintf(chain_file,"mchains_rank%d_%d.csv",rank,fn);
	...
	sprintf(chain_file_1,"mchainsc_rank_%d.csv",rank);
