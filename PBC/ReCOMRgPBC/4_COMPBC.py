...
for i in range (0,41):
	...
	szsc=array.array('d',(0.0 for c in range(0,36)))
	szss=array.array('d',(0.0 for c in range(0,36)))
	...
	tzsc=array.array('d',(0.0 for c in range(0,36)))
	tzss=array.array('d',(0.0 for c in range(0,36)))
	...
	for c in range(36):
		for b in range(20):
	       ...
	       # calulating cosine and sine of angles
			...
			szsc[c]+=math.cos(thetaz)
	
			sxss[c]+=math.sin(thetax)
			syss[c]+=math.sin(thetay)
			szss[c]+=math.sin(thetaz)

		print(c,sxsc[c],sxss[c],sysc[c],syss[c],szsc[c],szss[c],"\n")
