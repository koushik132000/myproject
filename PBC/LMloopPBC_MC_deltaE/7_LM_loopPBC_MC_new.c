...
//Modified Date:11Jan2025
	...
	//26-08-2025: Corrected deltaE subroutine for neighbor list calculation in y.
...
double deltaE(int olds,int news)
{
    ...
    // Build neighbor list for old site (with PBC)
    if((olds+1)%lx!=0) olist[0]=olds+1;
    else olist[0]=olds-(lx-1);

    if(olds%lx!=0) olist[1]=olds-1;
    else olist[1]=olds+(lx-1);
    ...
    // Build neighbor list for new site (with PBC)
    if((news+1)%lx!=0) nlist[0]=news+1;
    else nlist[0]=news-(lx-1);

    if(news%lx!=0) nlist[1]=news-1;
    else nlist[1]=news+(lx-1);
    ...
    // Count neighbors
    for (i=0;i<6;i++){
        if(olist[i]>=0 && olist[i]<ns) oldn += lcs.sz[olist[i]];
        if(nlist[i]>=0 && nlist[i]<ns) newn += lcs.sz[nlist[i]];
    }
    ...
    // Energy difference
    if(lcs.sz[news] >= 1){
        // If overlap → include Ex penalty, but still return Eb contribution too
        Ediff = lcs.sz[news]*Ex + (newn-oldn)*Eb;
    } else {
        Ediff = (newn-oldn)*Eb;
    }
...
int main()
{
	...
	do{			
		...
		printf("Chain %d bead %d: old neighbors=%d, new neighbors=%d\n",cnum,0,last_oldn, last_newn,dE);
		if(metrop(dEf)==1){	
			scalc=fpos.ex+lx*fpos.ey+lx*ly*fpos.ez;
		...
		printf("Chain %d bead %d: old neighbors=%d, new neighbors=%d\n",cnum, nb, last_oldn, last_newn,dE);
		if (metrop(dEl)==1){
			scalc=npos.ex+lx*npos.ey+lx*ly*npos.ez;	
		...
		printf("Chain %d bead %d: old neighbors=%d, new neighbors=%d\n",cnum, pb, last_oldn, last_newn,dE);
		if (metrop(dE)==1){
		...
		scalc=kpos.ex+lx*kpos.ey+lx*ly*kpos.ez;
		dE=deltaE(pcalc,scalc);
		printf("Chain %d bead %d: old neighbors=%d, new neighbors=%d\n",cnum, bnum, last_oldn, last_newn,dE);
		if (metrop(dE)==1){	
