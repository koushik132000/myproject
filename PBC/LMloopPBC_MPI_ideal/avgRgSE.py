# creating a file to calculate the convergence of root mean square radius of gyration
import pandas as pd
import numpy as np
import math
...
print(f"Final Rg = {fmean:.3f} ± {stderr:.3f}")
#df.to_csv("SE.csv", index=False, header=False)
df = pd.DataFrame([[fmean,stderr]],columns=["Mean_Rg", "StdErr"] ) # converting SE into list 
df.to_csv("avgRg.csv", index=False)